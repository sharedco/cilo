...
func TestEnvRenderExample(t *testing.T) {
	...
	fromPath := filepath.Join(moduleDir, "examples", "env-render")
...
func TestCiloBasicExample(t *testing.T) {
	...
	moduleDir, _ := moduleAndRepoRoots(t)
	fromPath := filepath.Join(moduleDir, "examples", "basic")
	if _, err := os.Stat(fromPath); err != nil {
		t.Fatalf("missing example: %v", err)
	}
...
func ciloBinaryPath() string {
	if binary := os.Getenv("CILO_BINARY"); binary != "" {
		return binary
	}
	return "cilo"
}

func runCilo(moduleDir string, args ...string) error {
	cmd := exec.Command(ciloBinaryPath(), args...)
	cmd.Dir = moduleDir
...
func runCiloOutput(moduleDir string, args ...string) (string, error) {
	cmd := exec.Command(ciloBinaryPath(), args...)
	cmd.Dir = moduleDir
